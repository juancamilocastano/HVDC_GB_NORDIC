function build_ac_opf_acdc_frequency_sev_var_res_provi_optimized!(m::Model)
    ...
    rg_lg1 = m.ext[:variables][:rg_lg1] = @variable(m, [g=Set_WWS1,t=T],lower_bound=0,upper_bound=upramp[g]*G_dt[g]/3600, base_name = "rg_lg1") # frequency reserve generators loss of generation area 1
    rg_lc1 = m.ext[:variables][:rg_lc1] = @variable(m, [g=Set_WWS1,t=T],lower_bound=0, upper_bound=upramp[g]*G_dt[g]/3600, base_name = "rg_lc1") # frequency reserve generators loss of converter area 1
    rg_lg2 = m.ext[:variables][:rg_lg2] = @variable(m, [g=Set_WWS2,t=T],lower_bound=0, upper_bound=upramp[g]*G_dt[g]/3600, base_name = "rg_lg2") # frequency reserve generators loss of generation area 2
    rg_lc2 = m.ext[:variables][:rg_lc2] = @variable(m, [g=Set_WWS2,t=T],lower_bound=0, upper_bound=upramp[g]*G_dt[g]/3600, base_name = "rg_lc2") # frequency reserve generators loss of converter area 2
    rg_l_reserve_1= m.ext[:variables][:rg_l_reserve_1] = @variable(m, [g=Set_WWS1,t=T],lower_bound=0,upper_bound=upramp[g]*G_dt[g]/3600, base_name = "rg_l_reserve_1") # frequency reserve generators loss of generation area 
    rg_l_reserve_2= m.ext[:variables][:rg_l_reserve_2] = @variable(m, [g=Set_WWS2,t=T],lower_bound=0, upper_bound=upramp[g]*G_dt[g]/3600, base_name = "rg_l_reserve_2") # frequency reserve generators loss of generation area 2
    ...
    zg = m.ext[:variables][:zg] = @variable(m, [g=Set_WWS,t=T],binary= true , base_name = "zg") # status variable generator
    betag = m.ext[:variables][:betag] = @variable(m, [g=Set_WWS,t=T], binary= true, base_name = "betag") #
    gammag = m.ext[:variables][:gammag] = @variable(m, [g=Set_WWS,t=T], binary= true, base_name = "gammag") #
   ...
   #Reservoir storage variables
    ...
    if max_gen_ncost == 1
        m.ext[:objective] = @objective(m, Min,
                sum(gen_cost[g][1]
                        for g in Set_WWS)
                            +sum(start_up_cost[g]*betag[g,t] for g in Set_WWS, t in T)
                            ...
                            +sum(G_reservecost[g]*rg_lg1[g,t] for g in Set_WWS1, t in T)*baseMVA
                            +sum(G_reservecost[g]*rg_lg2[g,t] for g in Set_WWS2, t in T)*baseMVA
                            +sum(G_reservecost[g]*rg_lc1[g,t] for g in Set_WWS1, t in T)*baseMVA
                            +sum(G_reservecost[g]*rg_lc2[g,t] for g in Set_WWS2, t in T)*baseMVA
                            +sum(G_reservecost[g]*rg_l_reserve_1[g,t] for g in Set_WWS1, t in T)*baseMVA
                            +sum(G_reservecost[g]*rg_l_reserve_2[g,t] for g in Set_WWS2, t in T)*baseMVA
    ...
    m.ext[:constraints][:event_binary_constraint]= @constraint(m, [g=Set_WWS1,t=T],
    zg[g,t]>=δg[g,t])
  ...
  m.ext[:constraints][:max_gen_power] = @constraint(m, [g=Set_WWS,t=T],
        pg[g,t] <= pmax[g]*zg[g,t]
        )#3.11

    m.ext[:constraints][:min_gen_power] = @constraint(m, [g=Set_WWS,t=T],
        pg[g,t]>= pmin[g]*zg[g,t]
        )#3.11

        m.ext[:constraints][:max_gen_power_rg_g_1] = @constraint(m, [g=Set_WWS1,t=T],
        pg[g,t]+ rg_lg1[g,t]<= pmax[g]*zg[g,t]
        )

        m.ext[:constraints][:min_gen_power_rg_g_1] = @constraint(m, [g=Set_WWS1,t=T],
         pg[g,t]+ rg_lg1[g,t]>= pmin[g]*zg[g,t]
        )

        m.ext[:constraints][:max_gen_power_rg_c_1] = @constraint(m, [g=Set_WWS1,t=T],
        pg[g,t]+ rg_lc1[g,t]<= pmax[g]*zg[g,t]
        )

        m.ext[:constraints][:min_gen_power_rg_c_1] = @constraint(m, [g=Set_WWS1,t=T],
         pg[g,t]+ rg_lc1[g,t]>= pmin[g]*zg[g,t]
        )

        m.ext[:constraints][:max_gen_power_rg_reserve_1] = @constraint(m, [g=Set_WWS1,t=T],
        pg[g,t]+ rg_l_reserve_1[g,t]<= pmax[g]*zg[g,t]
        )

        m.ext[:constraints][:min_gen_power_rg_reserve_1] = @constraint(m, [g=Set_WWS1,t=T],
         pg[g,t]+ rg_l_reserve_1[g,t]>= pmin[g]*zg[g,t]
        )

        m.ext[:constraints][:max_gen_power_rg_g_2] = @constraint(m, [g=Set_WWS2,t=T],
        pg[g,t]+ rg_lg2[g,t]<= pmax[g]*zg[g,t]
        )

        m.ext[:constraints][:min_gen_power_rg_g_2] = @constraint(m, [g=Set_WWS2,t=T],
         pg[g,t]+ rg_lg2[g,t]>= pmin[g]*zg[g,t]
        )

        m.ext[:constraints][:max_gen_power_rg_c_2] = @constraint(m, [g=Set_WWS2,t=T],
        pg[g,t]+ rg_lc2[g,t]<= pmax[g]*zg[g,t]
        )

        m.ext[:constraints][:min_gen_power_rg_c_2] = @constraint(m, [g=Set_WWS2,t=T],
         pg[g,t]+ rg_lc2[g,t]>= pmin[g]*zg[g,t]
        )

         m.ext[:constraints][:max_gen_power_rg_reserve_2] = @constraint(m, [g=Set_WWS2,t=T],
        pg[g,t]+ rg_l_reserve_2[g,t]<= pmax[g]*zg[g,t]
        )

        m.ext[:constraints][:min_gen_power_rg_reserve_2] = @constraint(m, [g=Set_WWS2,t=T],
...
#Storage constraints
 ...
 m.ext[:constraints][:initial_energy_value_reservoir] = @constraint(m, [g in G_reservoir],
    ...
    for t in T 
        Inertia_nadir_frequency_1[t]=sum((zg[g,t]-δg[g,t])*ic[g]*pmax[g] for g in Set_WWS1)
        Inertia_nadir_frequency_2[t]=sum((zg[g,t]-δg[g,t])*ic[g]*pmax[g] for g in Set_WWS2)
        Inertia_nadir_frequency_converter_1[t]=sum(zg[g,t]*ic[g]*pmax[g] for g in CV1)
        Inertia_nadir_frequency_converter_2[t]=sum(zg[g,t]*ic[g]*pmax[g] for g in CV2)
    end
    

  #bounding the FFR and FCR contributions of all generator andconverter assets
  m.ext[:constraints][:fcr_bound_gen_lg1]= @constraint(m, [g in Set_WWS1, t in T],
     rg_lg1[g,t]<=(1-δg[g,t])*upramp[g]*G_dt[g]/3600
    )
    m.ext[:constraints][:fcr_bound_gen_lc1]= @constraint(m, [g in Set_WWS1, t in T],
     rg_lc1[g,t]<=upramp[g]*G_dt[g]/3600
    )

    m.ext[:constraints][:fcr_bound_gen_lreserve_1]= @constraint(m, [g in Set_WWS1, t in T],
     rg_l_reserve_1[g,t]<=upramp[g]*G_dt[g]/3600
    )
    
    m.ext[:constraints][:fcr_bound_gen_lg2]= @constraint(m, [g in Set_WWS2, t in T],
     rg_lg2[g,t]<=(1-δg[g,t])*upramp[g]*G_dt[g]/3600
    )

    m.ext[:constraints][:fcr_bound_gen_lc2]= @constraint(m, [g in Set_WWS2, t in T],
     rg_lc2[g,t]<=upramp[g]*G_dt[g]/3600
    )
    m.ext[:constraints][:fcr_bound_gen_lreserve_2]= @constraint(m, [g in Set_WWS2, t in T],
    ...
    m.ext[:constraints][:reserve_bound_rg_lg1] = @constraint(m, [g in Set_WWS1, t in T],
    rg_lg1[g,t] <= MaxFreqDev[g]*zg[g,t]
    )

    m.ext[:constraints][:reserve_bound_rg_lc1] = @constraint(m, [g in Set_WWS1, t in T],
    rg_lc1[g,t] <= MaxFreqDev[g]*zg[g,t]
    )

    m.ext[:constraints][:reserve_bound_rg_l_reserve_1] = @constraint(m, [g in Set_WWS1, t in T],
    rg_l_reserve_1[g,t] <= MaxFreqDev[g]*zg[g,t]
    )

    m.ext[:constraints][:reserve_bound_rg_lg2] = @constraint(m, [g in Set_WWS2, t in T],
    rg_lg2[g,t] <= MaxFreqDev[g]*zg[g,t]
    )

    m.ext[:constraints][:reserve_bound_rg_lc2] = @constraint(m, [g in Set_WWS2, t in T],
    rg_lc2[g,t] <= MaxFreqDev[g]*zg[g,t]
    )

    m.ext[:constraints][:reserve_bound_rg_l_reserve_2] = @constraint(m, [g in Set_WWS2, t in T],
    ...
    #Power balance constraint
        ...
        sum(rs_lg1[s,t] for s in S1)+sum(re_lg1[e,t] for e in E1)+sum(rg_lg1[g,t] for g in Set_WWS1)+sum(rhvdc_lg1[cv,t] for cv in CV1)>=plg1[t]
    )
    m.ext[:constraints][:power_balance_lc1]= @constraint(m, [t in T],
      sum(rs_lc1[s,t] for s in S1)+sum(re_lc1[e,t] for e in E1)+sum(rg_lc1[g,t] for g in Set_WWS1)+sum(rhvdc_lc1[cv,t] for cv in CV1)>=plc1[t]
    )

     m.ext[:constraints][:power_balance_lg2]= @constraint(m, [t in T],
        sum(rs_lg2[s,t] for s in S2)+sum(re_lg2[e,t] for e in E2)+sum(rg_lg2[g,t] for g in Set_WWS2)+sum(rhvdc_lg2[cv,t] for cv in CV2)>=plg2[t]
    )
     m.ext[:constraints][:power_balance_lc2]= @constraint(m, [t in T],
        sum(rs_lc2[s,t] for s in S2)+sum(re_lc2[e,t] for e in E2)+sum(rg_lc2[g,t] for g in Set_WWS2)+sum(rhvdc_lc2[cv,t] for cv in CV2)>=plc2[t]
    )

    m.ext[:constraints][:power_balance_lreserve_1]= @constraint(m, [t in T],
        sum(rs_l_reserve_1[s,t] for s in S1)+sum(re_l_reserve_1[e,t] for e in E1)+sum(rg_l_reserve_1[g,t] for g in Set_WWS1)>=plreserve_1[t]
    )
    m.ext[:constraints][:power_balance_lreserve_2]= @constraint(m, [t in T],
        sum(rs_l_reserve_2[s,t] for s in S2)+sum(re_l_reserve_2[e,t] for e in E2)+sum(rg_l_reserve_2[g,t] for g in Set_WWS2)>=plreserve_2[t]
    )
   ...
   # Constraint frequency nadir generators and converters area 1
    ...
    zpg1[t]==sum(rg_lg1[g,t]/G_dt[g] for g in Set_WWS1)   
    )
    ...
    zpc1[t]==sum(rg_lc1[g,t]/G_dt[g] for g in Set_WWS1)
    )
    ...
    zpreserve1[t]==sum(rg_l_reserve_1[g,t]/G_dt[g] for g in Set_WWS1)
    )
    ...
    zpg2[t]==sum(rg_lg2[g,t]/G_dt[g] for g in Set_WWS2)   
    )
    ...
    zpc2[t]==sum(rg_lc2[g,t]/G_dt[g] for g in Set_WWS2)
    )
    ...
    zpreserve2[t]==sum(rg_l_reserve_2[g,t]/G_dt[g] for g in Set_WWS2)
    )
    ...
    plg1[t]<= 0.0000001+ sum(re_lg1[e,t] for e in E1)+sum(rs_lg1[s,t] for s in S1)+sum(rhvdc_lg1[cv,t] for cv in CV1)+sum(rg_lg1[g,t] for g in Set_WWS1)
    )
    m.ext[:constraints][:time_nadir_occurrence_g1_2]= @constraint(m, [t in T],
    plg1[t]>=0.0000001+ sum(re_lg1[e,t] for e in E1)+sum(rs_lg1[s,t] for s in S1)+sum(rhvdc_lg1[cv,t] for cv in CV1)+sum(rg_lg1[g,t] for g in Set_WWS1)*Edeployment["1"]/G_dt["1"]
    )

    m.ext[:constraints][:time_nadir_occurrence_c1_1]= @constraint(m, [t in T],
    plc1[t]<= 0.0000001+ sum(re_lc1[e,t] for e in E1)+sum(rs_lc1[s,t] for s in S1)+sum(rhvdc_lc1[cv,t] for cv in CV1)+sum(rg_lc1[g,t] for g in Set_WWS1)
    )
    m.ext[:constraints][:time_nadir_occurrence_c1_2]= @constraint(m, [t in T],
    plc1[t]>=0.0000001+ sum(re_lc1[e,t] for e in E1)+sum(rs_lc1[s,t] for s in S1)+sum(rhvdc_lc1[cv,t] for cv in CV1)+sum(rg_lc1[g,t] for g in Set_WWS1)*Edeployment["1"]/G_dt["1"]
    )

    m.ext[:constraints][:time_nadir_occurrence_g2_1]= @constraint(m, [t in T],
    plg2[t]<= 0.0000001 +sum(re_lg2[e,t] for e in E2)+sum(rs_lg2[s,t] for s in S2)+sum(rhvdc_lg2[cv,t] for cv in CV2)+sum(rg_lg2[g,t] for g in Set_WWS2)
    )
    m.ext[:constraints][:time_nadir_occurrence_g2_2]= @constraint(m, [t in T],
    plg2[t]>=0.0000001 +sum(re_lg2[e,t] for e in E2)+sum(rs_lg2[s,t] for s in S2)+sum(rhvdc_lg2[cv,t] for cv in CV2)+sum(rg_lg2[g,t] for g in Set_WWS2)*Edeployment["1"]/G_dt["1"]
    )

    m.ext[:constraints][:time_nadir_occurrence_c2_1]= @constraint(m, [t in T],
    plc2[t]<= 0.0000001 +sum(re_lc2[e,t] for e in E2)+sum(rs_lc2[s,t] for s in S2)+sum(rhvdc_lc2[cv,t] for cv in CV2)+sum(rg_lc2[g,t] for g in Set_WWS2)
    )
    m.ext[:constraints][:time_nadir_occurrence_c2_2]= @constraint(m, [t in T],
    plc2[t]>=0.0000001 +sum(re_lc2[e,t] for e in E2)+sum(rs_lc2[s,t] for s in S2)+sum(rhvdc_lc2[cv,t] for cv in CV2)+sum(rg_lc2[g,t] for g in Set_WWS2)*Edeployment["1"]/G_dt["1"]
    )

    m.ext[:constraints][:time_nadir_occurrence_reserve_1_1]= @constraint(m, [t in T],
    plreserve_1[t]<= 0.0000001 +sum(re_l_reserve_1[e,t] for e in E1)+sum(rs_l_reserve_1[s,t] for s in S1)+sum(rg_l_reserve_1[g,t] for g in Set_WWS1)
    )
    m.ext[:constraints][:time_nadir_occurrence_reserve_1_2]= @constraint(m, [t in T],
    plreserve_1[t]>=0.0000001 +sum(re_l_reserve_1[e,t] for e in E1)+sum(rs_l_reserve_1[s,t] for s in S1)+sum(rg_l_reserve_1[g,t] for g in Set_WWS1)*Edeployment["1"]/G_dt["1"]
    )

    m.ext[:constraints][:time_nadir_occurrence_reserve_2_1]= @constraint(m, [t in T],
    plreserve_2[t]<= 0.0000001 +sum(re_l_reserve_2[e,t] for e in E2)+sum(rs_l_reserve_2[s,t] for s in S2)+sum(rg_l_reserve_2[g,t] for g in Set_WWS2)
    )
    m.ext[:constraints][:time_nadir_occurrence_reserve_2_2]= @constraint(m, [t in T],
    plreserve_2[t]>=0.0000001 +sum(re_l_reserve_2[e,t] for e in E2)+sum(rs_l_reserve_2[s,t] for s in S2)+sum(rg_l_reserve_2[g,t] for g in Set_WWS2)*Edeployment["1"]/G_dt["1"]
    )
